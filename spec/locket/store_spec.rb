...
describe Locket::Store do
  include FakeFS::SpecHelpers
  before(:each) do
    path = '/dir/store/'
    @store = Locket::Store.new(path)
  ...
  describe '.initalize' do
    it 'should ensure root is saved as an absolute path' do
      store = Locket::Store.new('rel-root')
  ...
  describe 'each' do
    ...
    it 'should include hidden files iff :hidden is true' do
      ...
      expect(@store.each).to eq ['/dir/store/entry.ext']
      expect(@store.each(hidden: true)).to eq ['/dir/store/entry.ext',
                                               '/dir/store/.hidden.ext']
    end
  ...
  describe '[]' do
    it 'should be nil if the store does not exist?' do
      store = Locket::Store.new('/dne/test/locket/')
      expect(store.exist?).to be false
      expect(store['anything']).to eq false
    end
    it 'should be false if the store does not include? the name' do
      expect(@store.exist?)
      expect(@store['dne']).to eq false
    end
